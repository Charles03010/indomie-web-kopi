...
export const InputCafeDashboard = ({ name, url, onChange }: { name: string; url: string; onChange?: (e: React.ChangeEvent<HTMLInputElement>) => void }) => {
  const [preview, setPreview] = useState<string | null>(null);
  ...
  return (
    <label
      htmlFor={name}
      className="relative w-3xs aspect-square bg-(--input-bg) rounded-xl outline-0 flex items-center justify-center cursor-pointer 
                   overflow-hidden transition-colors duration-200
                   group"
    >
      <input
